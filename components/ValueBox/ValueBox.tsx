...
function ValueBox({ color, name, label }: Props) {
  ...
  const colorTop = `RGBA(${color[0]},${color[1]},${color[2]},${color[3]})`;
    return (
      ...
      <div className='value-box d-flex justify-content-center position-absolute'>
        {label&&<h3 className="position-absolute">{label}</h3>}
        <svg width="16vw" style={{transform: 'rotate(-30deg)', maxWidth: '150px'}} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" tabIndex="0">
